 ...
 */
Hopper::Hopper(UINT32 channel)
{
	m_HopperMotor = new Jaguar(channel);
	m_HopperMotor->Set(0);
}
 ...
 * @param channel The PWM channel on the digital module that the Jaguar is attached to.
 */
Hopper::Hopper(UINT32 slot, UINT32 channel) 
{
	m_HopperMotor = new Jaguar(slot, channel);
	m_HopperMotor->Set(0);
}
 ...
 */
void Hopper::Drive(float speed, bool enabled)
{
	if (enabled) {
		m_HopperMotor->Set(speed);
	} else {
