...
void CANJaguar::InitJaguar()
{
	...
	case kPercentVoltage:
		...
		break;
	case kCurrent:
		sendMessage(LM_API_ICTRL_T_EN | m_deviceNumber, NULL, 0);
		break;
	case kSpeed:
		sendMessage(LM_API_SPD_T_EN | m_deviceNumber, NULL, 0);
		break;
	default:
		return;
	}
 ...
 * In Current mode the value is in the range -1.0 to 1.0 (not in amps)
 * In Speed mode the value is floating point in rpm??
 * 
...
void CANJaguar::Set(float outputValue)
{
	...
	case kPercentVoltage:
		...
		INT16 value16 = (INT16)(outputValue * 32767.0);
		*((INT16*)dataBuffer) = swap16(value16);
		dataSize = sizeof(INT16);
		break;
	case kCurrent:
		// Docs say 32 bit (16.16) signed, but signal from BDComm shows
		// 16 bit signed integer.
		messageID = LM_API_ICTRL_T_SET  | m_deviceNumber;
		value16 = (INT16)(outputValue * 32767.0);
		*((INT16*)dataBuffer) = swap16(value16);
		dataSize = sizeof(INT16);
		break;
	case kSpeed:
		messageID = LM_API_SPD_T_SET  | m_deviceNumber;
		//TODO: *100.0 is temporary to map joystick -1 > 1 to -100 > 100
		INT32 value32 = (INT32)(outputValue*100.0*65536.0 );
...
float CANJaguar::Get()
{
	...
	case kPercentVoltage:
		...
		break;
	case kCurrent:
		messageID = LM_API_ICTRL_SET | m_deviceNumber;
		// Sending set with no data is a request for the last set
		sendMessage(messageID, NULL, 0);
		receiveMessage(&messageID, dataBuffer, &dataSize);
		if (dataSize == sizeof(INT32))
		{
			INT32 replyValue = *((INT32*)dataBuffer);
			return swap32(replyValue) / 256.0;
		}
		else
		{
			// TODO: Error
...
{
	if (m_controlMode == kPercentVoltage || m_controlMode == kCurrent)
	{
		Set(output);
	}
	else
