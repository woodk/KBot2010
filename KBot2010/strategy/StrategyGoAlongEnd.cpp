...
*/
StrategyGoAlongEnd::StrategyGoAlongEnd(KBot* kbot) : Strategy(kbot)
{
...
*/
StrategyGoAlongEnd::~StrategyGoAlongEnd()
{
...
*/
eState StrategyGoAlongEnd::apply()
{
    eState nNewState = knGoAlongEnd;    // assume we will keep running
    if (TargetInSight())
    {
...
*/
bool StrategyGoAlongEnd::getKeepMoving()
{
    bool bKeepMoving = true;
    if ((int)m_nCallCount/50.0 > GO_ALONG_END_TIME)
    {
        bKeepMoving = false;
    }
...
bool StrategyGoAlongEnd::TargetInSight()
{
